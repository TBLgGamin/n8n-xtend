...
function findElementByClassPattern(
  parent: Element,
...
export function inject(projectId: string): boolean {
  ...
  const sideMenu = findElementByClassPattern(
    sidebar,
    SIDEBAR_SELECTORS.sideMenu
  );
  if (!sideMenu) {
  ...
  const bottomMenu = findElementByClassPattern(
    sideMenu,
    SIDEBAR_SELECTORS.bottomMenu
  );
